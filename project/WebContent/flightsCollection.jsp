...
<script type="text/javascript">
  ...
  <script type="text/javascript">
			...
			function collect(tid,a){
				var ajax=new XMLHttpRequest();
				var result;
				ajax.open("POST","/project/collect");
				ajax.setRequestHeader("Content-type","application/x-www-form-urlencoded");
				ajax.send("type=cancelcollect&tid="+tid);
...
<%! 
	...
	GetCollectFlightServiceImpl result;
	int uid;
...
<body class="sarchbody">
    ...
    <table class="tablelist">
        ...
        <%
				...
				*/
				...
				result=new GetCollectFlightServiceImpl();
				flights=result.getFlightsByPage(uid,start,end,takeoff,land,maxPrice,pageNo);
				if(flights!=null){
					for(Flight each:flights){
						out.write("<tr><td>"+each.getName()+"</td><td>"+each.getStart()+"</td><td>"+each.getEnd()+"</td><td>"+each.getTakeoff()+"</td><td>"+each.getLand()+"</td><td>"+each.getPrice()+"</td><td>"+"<a href=\"#\" onclick=\"collect("+each.getTid()+")\">取消收藏</a>"+"</td></tr>");
					}
				}
    ...
    <div class="pagin" >
    	<div class="message">共<i class="blue"><%=result.getMaxItem(uid,start,end,takeoff,land,maxPrice) %></i>条记录，当前显示第&nbsp;<i class="blue"><%=pageNo %>&nbsp;</i>页</div>
        ...
        <%
        	...
        	int maxpage=result.getMaxPage(uid,start, end, takeoff, land, maxPrice);
       		 int i;
